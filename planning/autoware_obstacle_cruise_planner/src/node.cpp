...
double calcDistanceToFrontVehicle(
  ...
  const geometry_msgs::msg::Point & obstacle_pos)
{
  ...
  if (obstacle_idx == 0) {
    auto long_projected_distance_to_trajectory_point = [&]() -> double {
      const auto target_point = obstacle_pos;
      const auto traj_point = traj_points.at(obstacle_idx);

      const auto tp_yaw = tf2::getYaw(traj_point.pose.orientation);
      const Eigen::Vector2d tp_vec_norm(cos(tp_yaw), sin(tp_yaw));
      const Eigen::Vector2d relative_to_tp_vec(
        target_point.x - traj_point.pose.position.x, target_point.y - traj_point.pose.position.y);

      // Calculate the projection of relative_to_tp_vec onto the direction vector tp_vec_norm
      const double projected_distance = relative_to_tp_vec.dot(tp_vec_norm);
      return projected_distance;
    };
    const double total_dist =
      long_projected_distance_to_trajectory_point() + ego_to_obstacle_distance;
    return total_dist;
  }
...
std::vector<PredictedPath> resampleHighestConfidencePredictedPath(
 ...
 */
std::pair<double, double> calculateTrajectoryApproachVelocity(
  ...
  const geometry_msgs::msg::Twist & obstacle_twist)
{
  ...
  std::cout << "projected_velocity[0]: " << projected_velocity[0]
            << " obstacle_twist: " << obstacle_twist.linear.x << std::endl;
  ...
  const std::vector<TrajectoryPoint> & traj_points) const
{
  ...
  for (const auto & predicted_object : objects.objects) {
    ...
    const auto projected_vel = calculateTrajectoryApproachVelocity(
    ...
    if (ego_to_obstacle_distance < 0.0) {
      // check its tangent velocity high or not
      if (
        (odometry.twist.twist.linear.x < 0.1 && projected_vel.first < 0.3) ||
        projected_vel.first < odometry.twist.twist.linear.x || ego_to_obstacle_distance < -10.0) {
        RCLCPP_INFO_EXPRESSION(
          get_logger(), enable_debug_info_, "Ignore obstacle (%s) since it is behind the ego.",
          object_id.c_str());
        continue;
      }
    }
    ...
    for (const auto & cluster_indices : clusters) {
      ...
      for (const auto & index : cluster_indices.indices) {
        ...
        if (min_lat_dist_to_traj_poly < max_lat_margin) {
          ...
          if (current_ego_to_obstacle_distance >= 0.0) {
  ...
  const std::vector<TrajectoryPoint> & traj_points, const std::vector<Obstacle> & obstacles)
{
  ...
  std::cout << "current vehicle speed: " << odometry.twist.twist.linear.x << std::endl;
  ...
  const double precise_lat_dist)
{
  ...
  if (obstacle.lon_velocity < 0.0) {
    RCLCPP_INFO_EXPRESSION(
      get_logger(), enable_debug_info_,
      "[Cruise] Ignore obstacle (%s) since it's driving opposite direction.", object_id.c_str());
    return std::nullopt;
  }

  if (p.max_lat_margin_for_cruise < precise_lat_dist) {
    const auto time_to_traj = precise_lat_dist / std::max(1e-6, obstacle.lat_velocity);

    if (time_to_traj < 0.0 || time_to_traj > 2.5) {
      {
        RCLCPP_INFO_EXPRESSION(
          get_logger(), enable_debug_info_,
  ...
  const auto collision_points = [&]() -> std::optional<std::vector<PointWithStamp>> {
    constexpr double epsilon = 1e-6;
    if (precise_lat_dist < epsilon && obstacle.ego_to_obstacle_distance > 0.0) {
    ...
    return createCollisionPointsForOutsideCruiseObstacle(
      odometry, traj_points, traj_polys, obstacle);
  }();
  if (!collision_points) {
    return std::nullopt;
  }
  ...
  return CruiseObstacle{obstacle.uuid,         obstacle.stamp,        obstacle.pose,
                        obstacle.lon_velocity, obstacle.lat_velocity, *collision_points};
}
  ...
  const std::vector<Obstacle> & obstacles, const std::vector<TrajectoryPoint> & traj_points)
{
  ...
  std::for_each(
    ...
    [&stopped_obstacles, &moving_obstacles, &p](const auto & o) {
      const auto is_front = o.ego_to_obstacle_distance >= 0.0;
      if (!is_front) return;
  ...
  const Obstacle & obstacle) const
{
  ...
  const auto collision_points = polygon_utils::getCollisionPoints(
    traj_points, traj_polys, obstacle.stamp, resampled_predicted_path.front(), obstacle.shape,
  ...
  const std::vector<Polygon2d> & traj_polys, const Obstacle & obstacle) const
{
  ...
  // if ego is stopping, check only stop obstacles
  if (odometry.twist.twist.linear.x < 0.1) {
    RCLCPP_INFO_EXPRESSION(
      get_logger(), enable_debug_info_,
      "[Cruise] Ignore outside obstacle (%s) since the ego is stopping.", object_id.c_str());
    return std::nullopt;
  }
  ...
  const auto resampled_predicted_path = resampleHighestConfidencePredictedPath(
    obstacle.predicted_paths, p.prediction_resampling_time_interval,
    p.prediction_resampling_time_horizon, 3);
  ...
  const auto getCollisionPoints = [&]() -> std::vector<PointWithStamp> {
    for (const auto & predicted_path : resampled_predicted_path) {
  ...
  const PredictedPath & resampled_predicted_path, double max_lat_margin_for_stop) const
{
  ...
  if (!isOutsideStopObstacle(obstacle.classification.label)) {
    RCLCPP_INFO_EXPRESSION(
      get_logger(), enable_debug_info_,
      "[Stop] Ignore outside obstacle (%s) since its type is not designated.", object_id.c_str());
    return std::nullopt;
  }
  std::vector<size_t> collision_index;
  const auto collision_points = polygon_utils::getCollisionPoints(
  ...
  const double precise_lat_dist) const
{
  ...
  if (
    ...
    obstacle.ego_to_obstacle_distance < 0.0) {
//    const auto time_to_traj = precise_lat_dist / std::max(1e-6, obstacle.lat_velocity);
    // ignore if the obstacle is moving away (time_to_traj < 0) or too far (time_to_traj > 1)
//    if (time_to_traj < 0.0 || time_to_traj > 2.0) {
//      RCLCPP_INFO_EXPRESSION(
//        get_logger(), enable_debug_info_,
//        "[Stop] Ignore outside obstacle (%s) since it's far from trajectory.", object_id.c_str());
//      std::cout << "time_to_traj: " << time_to_traj << std::endl;
//      return std::nullopt;
//    }
    const auto collision_point = createCollisionPointForOutsideStopObstacle(
      odometry, traj_points, traj_polys, obstacle, resampled_predicted_path.front(),
      max_lat_margin_for_stop);

    if (collision_point) {
      return StopObstacle{obstacle.uuid,         obstacle.stamp,         obstacle.classification,
                          obstacle.pose,         obstacle.shape,         obstacle.lon_velocity,
                          obstacle.lat_velocity, collision_point->first, collision_point->second};
    }
    return std::nullopt;
  }
  ...
  const double time_to_reach_collision_point = calcTimeToReachCollisionPoint(
    odometry, collision_point->first, traj_points, abs_ego_offset, 3.0);

  const bool is_transient_obstacle = [&]() {
    // get the predicted position of the obstacle when ego reaches the collision point
    if (resampled_predicted_path.empty() || resampled_predicted_path.front().path.empty()) {
      return false;
    }
    const auto future_obj_pose = object_recognition_utils::calcInterpolatedPose(
      resampled_predicted_path.front(), time_to_reach_collision_point);

    Obstacle tmp_future_obs = obstacle;
    tmp_future_obs.pose =
      future_obj_pose ? future_obj_pose.value() : resampled_predicted_path.front().path.back();
  ...
  const std::vector<TrajectoryPoint> & traj_points, const bool is_driving_forward) const
{
  ...
  const bool is_ego_stopping = odometry.twist.twist.linear.x < 0.1;

  const double time_to_reach_collision_point =
    is_ego_stopping ? calcTimeToReachCollisionPoint(
                        odometry, collision_points.front().point, traj_points, abs_ego_offset, 10.0)
                    : calcTimeToReachCollisionPoint(
                        odometry, collision_points.front().point, traj_points, abs_ego_offset, 0.0);
  ...
  std::cout << "is_ego_stopping: " << is_ego_stopping
            << ", time_to_reach_collision_point: " << time_to_reach_collision_point
            << ", time_to_start_cross: " << time_to_start_cross
            << ", time_to_end_cross: " << time_to_end_cross << std::endl;
...
double ObstacleCruisePlannerNode::calcTimeToReachCollisionPoint(
  ...
  const double min_transient_obj_distance) const
{
  const double dist_from_ego_to_obstacle =
    ...
    abs_ego_offset;
  return dist_from_ego_to_obstacle > min_transient_obj_distance
           ? dist_from_ego_to_obstacle / std::max(1e-6, std::abs(odometry.twist.twist.linear.x))
           : 0.0;
}
}  // namespace autoware::motion_planning
